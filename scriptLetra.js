...
const textInputEl = document.getElementById('titulo');
...
// String to show
let string = 'Feliz<div>21th September</div> <div>AMY...!!!  </div>';
const tituloEl = document.getElementById('titulo');
tituloEl.innerHTML = string;
...
textInputEl.setAttribute('contenteditable', 'false');
textInputEl.focus();
...
function init() {
    ...
    orbit.enableRotate = false; // Desactiva la rotación
    orbit.enableZoom = false;   // Desactiva el zoom
...
function refreshText() {
    // Muestra el texto en el canvas 2D
    sampleCoordinates();

    // Mapea las coordenadas a partículas
    ...
    // Recrea la malla instanciada
    recreateInstancedMesh();
    // Ajusta la cámara para que el texto se ajuste a la pantalla
    makeTextFitScreen();
...
function sampleCoordinates() {
    ...
    if (stringBox.wTexture > 0) {
        ...
        if (textureCoordinates.length !== 0) {
            // Limpieza: eliminar coordenadas y partículas que desaparecieron en el paso anterior
            textureCoordinates = textureCoordinates.filter(c => !c.toDelete);
            particles = particles.filter(c => !c.toDelete);

            // Recorre las coordenadas existentes
